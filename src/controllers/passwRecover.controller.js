...
import { passwRecoverService } from '../services/passwRecover.service.js';

class RecoveryController {
  async sendEmail(req, res) {
    try {
      const { email } = req.body;
      const result = await passwRecoverService.sendEmail(email);
      if (result !== null) {
  ...
  async findToken(req, res) {
    try {
      ...
      const foundToken = await passwRecoverService.findToken(token, email);
  ...
  async newPassword(req, res) {
    try {
      ...
      const foundUser = await passwRecoverService.newPassword(userEmail, password, token);
...
export const passwRecoverController = new RecoveryController();
